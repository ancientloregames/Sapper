...
class GameView extends SurfaceView implements Runnable {
    ...
    private void draw(){
        if (surfaceHolder.getSurface().isValid()) {
            ...
            canvas.drawText("Flags: " + _lm.getFlagsCount(), 20,
                    upperPanelY/2+paint.getTextSize()/2, paint);
            if (_gm.isDebug())
                canvas.drawText("Mines: " + _lm.getMinesCount(),paint.getTextSize()*5 + 20,
                        upperPanelY/2+paint.getTextSize()/2, paint);
            ...
            if (_gm.getState()==GameState.PAUSE){
                ...
                canvas.drawText("Tutorial",displayX/2,displayY/2+upperPanelY,paintState);
                canvas.drawText("Tap on tiles to open them. Digits will show,",
                        displayX/2,displayY/2+upperPanelY+paintState.getTextSize()+5,paintTip);
                canvas.drawText("how many mines are around. Toggle button FLAG,",
                        displayX/2,displayY/2+upperPanelY+paintState.getTextSize()+5+paintTip.getTextSize(),paintTip);
                canvas.drawText("to mark potential mines. Good luck;)",
    ...
    public void run() {
        while (running){
            if(_gm.getState()==GameState.PLAYING) _lm.update(_gm,_sm);
            draw();
            try {
